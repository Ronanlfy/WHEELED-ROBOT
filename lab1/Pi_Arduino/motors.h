...
private:
  bool INVERSE;
  double Kp, Ki, Kd, scale = 100;
  unsigned long time_prev, time_now;
  double angular_vel, time_diff, err_prev = 0, err_integ = 0;
  double TICS_PER_ROTATION = 192; //This can be varied based on interrupt configuration
  int current_pwm = 0;
  double thres = 1000, deadzone = 0.05;
public:
...
void Motor::rotate(double ref, int direction){
  ...
  Motor::pid_controller(ref * (double)direction);
  /*if (current_pwm < 0){
